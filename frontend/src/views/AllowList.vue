...
export default {
  ...
  methods: {
    async initialize() {
      const allowList = 'http://'+ config.SERVER +':'+ config.PORT+'/api/allow_list'
      let initialData = await axios.get(allowList)
    ...
    disableAll(){
      if(this.disableBtn === "Disable All") {
       this.disableBtn = "Allow All"
        axios.post('http://'+ config.SERVER +':'+ config.PORT+'/api/allow_list/disable')
        this.initialize()
      }
      else {
        this.disableBtn = "Disable All"
        axios.post('http://'+ config.SERVER +':'+ config.PORT+'/api/allow_list/enable')
        this.initialize()
      }
    },
    editItem(item) {
    ...
    update(oldData, newData) {
      console.log(newData)
      axios.post('http://'+ config.SERVER +':'+ config.PORT+'/api/allow_list/update', {
        oldData: oldData,
        newData: newData
      })
    },
    insert(data) {
      axios.post('http://'+ config.SERVER +':'+ config.PORT+'/api/allow_list/insert', {
        data: data
      })
    },
    delete(data) {
      axios.post('http://'+ config.SERVER +':'+ config.PORT+'/api/allow_list/delete', {
        data: data
      })
    }
  }
